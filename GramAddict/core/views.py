...
class SearchView:
    ...
    def _clear_search_bar(self, search_edit_text) -> None:
        """Svuota la searchbar PRIMA di digitare.

        Senza questo il testo del target precedente puo' restare nel campo e il
        nuovo si appiccica in coda (visto nei log: cercando
        'manuel_santalucia_ifbbpro' la searchbar conteneva
        'gretanicoletti_ptnuel_santalucia_ifbbpro') -> la ricerca non trova
        nulla e la sorgente viene contata come morta.

        Ordine dei tentativi: PRIMA ``set_text("")``, che passa da
        ``jsonrpc.clearTextField`` e non tocca la tastiera. NON usare per primo
        ``deviceV2.clear_text()``: quello chiama ``wait_fastinput_ime()`` (fino a
        5s di attesa) e forza l'IME su FastInputIME -- proprio la tastiera rotta
        su questi emulatori per cui esiste ``_adb_type_text``.
        """
        try:
            search_edit_text.set_text("", Mode.PASTE)
            return
        except Exception as e:
            logger.debug(f"_clear_search_bar: set_text('') fallita: {e}")
        try:
            self.device.deviceV2.clear_text()
        except Exception as e:
            logger.debug(f"_clear_search_bar: clear_text fallita: {e}")

    def _type_target_and_readback(self, search_edit_text, target: str) -> str:
        """Digita ``target`` nella searchbar e rilegge il campo.

        Ritorna:
          - "ok"       : la searchbar contiene esattamente l'handle
          - "mismatch" : contiene ALTRO (digitazione persa, placeholder 'Search',
                         testo concatenato) -> cercare ora significherebbe
                         guardare i risultati di un testo diverso
          - "unknown"  : campo non rileggibile (non decidiamo nulla, si prosegue)
        """
        ...
        if not adb_typed:
            ...
            logger.warning(f"⌨️  adb-type fallita, fallback a set_text({mode_name}).")
            ...
            if args.dont_type:
                logger.info(
                    "⌨️  Kick-search: invio SPACE+BACKSPACE per forzare live-filter."
                )
                self._kick_search()
        # Give IG some time to fetch search results
        random_sleep(2, 4, modulable=False)
        try:
            # NB: riusiamo il selector ricevuto (e' lazy, si risolve ora) invece di
            # richiamare _getSearchEditText(): quello, se non trova il campo entro
            # Timeout.LONG, fa close_keyboard + Home + Search e azzererebbe query e
            # risultati proprio mentre stiamo leggendo.
            if search_edit_text is None or not search_edit_text.exists():
                return "unknown"
            seen = search_edit_text.get_text(error=False) or ""
        except Exception as e:
            logger.debug(f"navigate_to_target: could not read searchbar text: {e}")
            return "unknown"
        if not seen:
            return "unknown"
        if seen.strip().casefold() == target.strip().casefold():
    ...
    def navigate_to_target(
        ...
        self._clear_search_bar(search_edit_text)
        typed = self._type_target_and_readback(search_edit_text, target)
        if typed == "mismatch":
            # 18 MISMATCH nello storico dei log, 0 poi risolti con successo: se il
            # campo non contiene l'handle, la ricerca lavora su un testo diverso.
            # Un retry pulito recupera i casi in cui la digitazione e' andata
            # persa; se fallisce ancora, meglio saltare che cercare alla cieca.
            logger.warning(f"⌨️  Ripulisco e ridigito {target!r} (1 retry).")
            self._clear_search_bar(search_edit_text)
            typed = self._type_target_and_readback(search_edit_text, target)
            if typed == "mismatch":
                logger.warning(
                    f"⛔ @{target}: la searchbar non contiene l'handle dopo 2 tentativi. "
                    "Salto la sorgente invece di cercare alla cieca."
                )
                _set_last_nav_failure(NAV_FAIL_TYPING)
                UniversalActions.close_keyboard(self.device)
                return False
        ...
        # know whether typing hit the right widget at all. Usiamo il selector che
        # abbiamo gia' (vedi sopra: _getSearchEditText() puo' resettare la ricerca)
        # e error=False perche' una riga di debug non deve poter sollevare.
        try:
            if search_edit_text is not None and search_edit_text.exists():
                logger.debug(
                    "navigate_to_target: searchbar currently contains: "
                    f"{search_edit_text.get_text(error=False)!r}"
                )
        except Exception as e:
            logger.debug(f"navigate_to_target: could not read searchbar text: {e}")
        ...
        if self._check_current_view(target, job, in_place_tab=True):
            return self._verify_opened_profile(target, job)
        _set_last_nav_failure(NAV_FAIL_NOT_FOUND)
        return False
